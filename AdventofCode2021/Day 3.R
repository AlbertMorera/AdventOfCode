...
imp <- c("00100", "11110", "10110", "10111", "10101", "01111", "00111", "11100", "10000", "11001", "00010", "01010")

m <- sapply(imp, function(x) substring(x, first=1:5, last=1:5))
class(m) <- "numeric"
...
gamma.rate.dec <- strtoi(paste(gamma.rate, collapse = ''), base = 2)
epsilon.rate.dec <- strtoi(paste(epsilon.rate, collapse = ''), base = 2)
